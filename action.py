...
def fatorial(n : int) -> str:
    fatorial_regex = re.compile(r'\d+\!|^\d\!|^\d\!$|^\d\!$', flags=re.S)
...
def solve_exponentiations(equation: str) -> str:
    exponentiation_regex = re.compile(r'\d+\.?\d*(?:\^|\*\*)\d+\.?\d*', flags=re.S)
